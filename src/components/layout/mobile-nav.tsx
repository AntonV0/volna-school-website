...
export function MobileNav({ locale }: MobileNavProps) {
  ...
  return (
    <div className="lg:hidden">
      ...
      {isOpen ? (
        <div className="absolute inset-x-4 top-20 z-40 rounded-lg bg-brand-teal p-5 text-white shadow-2xl">
            ...
            <nav aria-label="Mobile primary">
              <ul className="grid gap-2">
                {content.navigation.primary.map((routeKey) => (
                  <li key={routeKey}>
                    <Link
                      className="block rounded-md px-3 py-3 text-base font-semibold transition hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
